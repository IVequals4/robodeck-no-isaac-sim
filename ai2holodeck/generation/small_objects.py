...
class SmallObjectGenerator:
    def __init__(self, object_retriever: ObjathorRetriever, llm: OpenAI):
        ...
        self.multiprocessing = False  # disabled to avoid torch shared memory issues
    ...
    def generate_small_objects(self, scene, controller, receptacle_ids):
        ...
        for i, (receptacle, small_objects) in enumerate(receptacle2small_objects.items()):
            print(f"Placing small objects: receptacle {i+1}/{total_receptacles}: {receptacle} ({len(small_objects)} objects)")
            placements = []

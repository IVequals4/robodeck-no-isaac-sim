...
class ObjectSelector:
    ...
    def get_asset_components(self, asset_id):
        """Extract component types present in a composite asset using description_auto."""
        annotation = get_annotations(self.database[asset_id])
        
        description_auto = annotation.get("description_auto") or ""
        description = annotation.get("description") or ""
        combined = f"{description_auto} {description}".lower()
        
        found = set()
        for component, keywords in COMPOSITE_OBJECT_KEYWORDS.items():
    ...
    def get_floor_objects(
        ...
        covered_by_composite = set()  # object types already present in selected assets
        floor_object_list = sorted(
            floor_object_list,
            key=lambda obj: (obj.get("size") or [0, 0, 0])[0] * (obj.get("size") or [0, 0, 0])[2],
            reverse=True
        )
        for floor_object in floor_object_list:
            ...
            object_type_normalized = object_type.lower().replace("_", " ")

            # check if this object type is already present in a previously selected composite
            if any(
                kw in object_type_normalized
                for component in covered_by_composite
                for kw in COMPOSITE_OBJECT_KEYWORDS.get(component, [])
            ):
                print(f"Skipping '{object_type}' — already present in a composite asset")
                continue
            ...
            for asset_id in set(selected_asset_ids):  # set() avoids duplicate lookups for "same"
                components = self.get_asset_components(asset_id)
                covered_by_composite.update(components)

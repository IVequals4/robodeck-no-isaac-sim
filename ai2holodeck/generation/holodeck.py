...
from ai2holodeck.generation.utils import get_top_down_frame, room_video, LLMWrapper, capture_progressive_top_down_frames, get_room_images, get_all_room_top_down_frames
...
from ai2holodeck.generation.optimise_scene import run_room_check
...
class Holodeck:
    ...
    def generate_scene(
        ...
        scene["floor_objects"], scene["scene_graph"]  = self.floor_object_generator.generate_objects(
            scene, use_constraint=use_constraint
        )

        if generate_image:
            scene_floor_only = copy.deepcopy(scene)
            scene_floor_only["objects"] = scene["floor_objects"]
            img = get_top_down_frame(scene_floor_only, self.objaverse_asset_dir, 1024, 1024)
            img.save(os.path.join(save_dir, f"{query_name}_stage1_floor.png"))
        ...
        # save top down image
        if generate_image:
            # Original single top-down image (full scene)
            top_image = get_top_down_frame(scene, self.objaverse_asset_dir, 1024, 1024)
            top_image.save(os.path.join(save_dir, f"{query_name} ai2thor.png"))

            # Progressive captures
            # capture_progressive_top_down_frames(
            #     scene=scene,
            ...
            room_images = get_room_images(scene, objaverse_asset_dir=self.objaverse_asset_dir)

            # Save them
            for room_name, images in room_images.items():
                for i, img in enumerate(images):
                    img.save(f"{save_dir}/{room_name}_corner_{i}.png")
        ...
        run_room_check(save_dir, "gpt-4o")
